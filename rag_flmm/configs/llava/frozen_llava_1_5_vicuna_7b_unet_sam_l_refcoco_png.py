import torch
try:
    # Prefer vendored implementation bundled with this project
    from flmm_llava.modeling_llava import CustomLlavaForConditionalGeneration
except Exception:
    # Fallback to external llava package if available in the environment
    from llava.modeling_llava import CustomLlavaForConditionalGeneration
from mmdet.datasets import RefCocoDataset
from mmdet.datasets.transforms import LoadAnnotations
from mmdet.models import CrossEntropyLoss, DiceLoss
...
try:
    # Prefer router-enabled inference class when available
    from flmm.models.finetune_llava import FrozenLlavaSAM
except Exception:
    from flmm.models.frozen_llava import FrozenLlavaSAM
...
model = dict(
    ...
    tokenizer=tokenizer,
    # LoRA-related knobs are optional in this inference config
    llm_lora=None,
...
datasets_list = [
    dict(
        type=PNGDataset,
        json_file="/home/hrkim/dataset/coco/annotations/png_coco_train2017.json",
        panoptic_json_file="/home/hrkim/dataset/coco/annotations/panoptic_train2017.json",
        panoptic_png_path="/home/hrkim/dataset/coco/annotations/panoptic_train2017",
        tokenizer=tokenizer,
        image_processor=image_processor,
        prompt_template=prompt_template,
        local_path="/home/hrkim/dataset/coco/train2017",
        ceph_path="openmmlab:s3://openmmlab/datasets/detection/coco/train2017",
        prompt=prompt,
    ),
    dict(
        type=RefCocoDataset,
        data_root="/home/hrkim/dataset/coco/",
...
train_dataloader = dict(
    ...
    dataset=dict(type=concat_datasets, datasets_list=datasets_list),
