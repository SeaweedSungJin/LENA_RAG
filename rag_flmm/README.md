...
The repository now provides an optional bridge to the encyclopedic VQA RAG pipeline
located under `../wikipedia`.  Use `integration.WikipediaRAGBridge` to run retrieval
and (optionally) answer generation, or launch the end-to-end demo script:

```bash
python3 scripts/run_wikipedia_rag_demo.py \
    --flmm-config configs/llava/frozen_llava_1_5_vicuna_7b_unet_sam_l_refcoco_png.py \
    --flmm-checkpoint /path/to/iter_xxxxx.pth \
    --wikipedia-root ../wikipedia \
    --wikipedia-config config.yaml \
    "What is the monument in this picture?" \
    /path/to/image.jpg
```

The demo will run the FrozenLlavaSAM router, invoke the Wikipedia search pipeline only
when the model predicts `[RAG]`, and report the retrieved sections and generated answer.
