...
class MGMMetaModel:
            ...
            if "model" in mm_projector_weights.keys():
                ...
                if is_deepspeed_zero3_enabled():
                    if len(mm_projector_weights) > 0:
                        with deepspeed.zero.GatheredParameters(
                            mm_projector_weights, modifier_rank=0
                        ):
                            if torch.distributed.get_rank() == 0:
                                self.mm_projector.load_state_dict(mm_projector_weights)
                else:
                    status = self.mm_projector.load_state_dict(
                ...
                if is_deepspeed_zero3_enabled():
                    ...
                    if len(named_parameters) > 0:
                        with deepspeed.zero.GatheredParameters(
                            named_parameters, modifier_rank=0
                        ):
                            if torch.distributed.get_rank() == 0:
    ...
    def initialize_uni_modules(self, model_args, for_eval=False):
        ...
        self.config.optimize_vision_tower = getattr(
            model_args, "optimize_vision_tower", False
        )
        self.config.optimize_vision_tower_aux = getattr(
            model_args, "optimize_vision_tower_aux", False
        )

        self.vlm_uni_query_projector = nn.Sequential(
            nn.LayerNorm(self.config.mm_hidden_size),
        ...
        if pretrain_mm_mlp_adapter is not None:
            ...
            if IS_NEW_TRANSFORMERS:
                try:
                    weight_file = json.load(
                        open(
                            os.path.join(
                                model_idx_path, "model.safetensors.index.json"
                            ),
                            "r",
                        )
                    )["weight_map"]
                ...
                weight_file = json.load(
                    open(
                        os.path.join(model_idx_path, "pytorch_model.bin.index.json"),
                        "r",
                    )
                )["weight_map"]
            model_path = set(
            ...
            for _model in model_path:
                if not IS_NEW_TRANSFORMERS:
                    projector_weights.update(
                        torch.load(
                            os.path.join(model_idx_path, _model), map_location="cpu"
                        )
                    )
                else:
                    with safetensors.safe_open(
                        os.path.join(model_idx_path, _model),
                        framework="pt",
                        device="cpu",
                    ) as f:
                        for _key in f.keys():
        ...
        def get_w(weights, keyword, main_module, sub_module):
            ...
            if is_deepspeed_zero3_enabled():
                named_parameters = [
                    v for k, v in getattr(main_module, sub_module).named_parameters()
                ]
                if len(named_parameters) > 0:
                    ...
                    with deepspeed.zero.GatheredParameters(
                        ...
                        if torch.distributed.get_rank() == 0:
                            getattr(main_module, sub_module).load_state_dict(
                                pretrain_weight
                            )
                    with deepspeed.zero.GatheredParameters(
                        self.mm_projector[0].weight, modifier_rank=None
                    ):
            ...
            if (
                ...
                or weight_type == torch.int8
                or weight_type == torch.int16
            ):
        ...
        if self.config.optimize_vision_tower_aux:
            ...
            get_w(
                projector_weights,
                "vision_tower_aux.vision_stem",
                self.vision_tower_aux,
                "vision_stem",
            )
            get_w(
                projector_weights,
                "vision_tower_aux.vision_stages",
                self.vision_tower_aux,
                "vision_stages",
            )
        get_w(
            projector_weights,
            "vlm_uni_query_projector",
            self,
            "vlm_uni_query_projector",
        )
...
class MGMMetaForCausalLM(ABC):
    ...
    def encode_images(self, images, images_aux=None, is_video=False):
        ...
        if images_aux is not None:
            image_aux_features_raw = (
                self.get_model()
                .get_vision_tower_aux()(images_aux)
                .to(dtype=image_features.dtype, device=image_features.device)
            )

            if image_global:
                image_aux_features_global = F.interpolate(
                    image_aux_features_raw.float(),
                    scale_factor=1 / image_grid,
                    mode="bilinear",
                    align_corners=False,
                ).to(dtype=image_aux_features_raw.dtype)
                image_feat_global, image_aux_feat_global = self.unified_resampler(
                    image_feat_global, image_aux_features_global
                )

            if image_grid > 1:
                image_aux_features_raw = image_aux_features_raw.reshape(
                    *image_aux_features_raw.shape[:2],
                    image_grid,
                ...
                image_aux_features_raw = (
                    image_aux_features_raw.permute(0, 2, 4, 1, 3, 5)
                    .flatten(1, 2)
                    .flatten(0, 1)
                    .contiguous()
                )
            image_features, image_aux_features = self.unified_resampler(
                image_features, image_aux_features_raw
            )

            if image_grid > 1:
                image_features = image_features.reshape(
                    batch_size, image_grid**2, *image_features.shape[1:]
                )
                image_features = image_features.flatten(1, 2).contiguous()
                image_aux_features = image_aux_features.reshape(
                    batch_size, image_grid**2, *image_aux_features.shape[1:]
                )
                image_aux_features = image_aux_features.flatten(1, 2).contiguous()

                # This is different from llava-next, llava-next re-obtains the original image format

            # add global features, [global, local]
            if image_global:
                image_features = torch.cat([image_feat_global, image_features], dim=1)
                image_aux_features = torch.cat(
                    [image_aux_feat_global, image_aux_features], dim=1
                )
    ...
    def unified_resampler(self, images, images_aux):
        ...
        images_aux = images_aux.reshape(
            len(images_aux), patch_num**2, patch_size**2, images_aux.shape[-1]
        ).contiguous()
        ...
        embed_att = embed_query[:, :, None] @ (
            embed_aux.transpose(-1, -2) / (embed_aux.shape[-1] ** 0.5)
        )
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        images=None,
        images_aux=None,
        mask_ids=None,  # todo: return mask_ids and image_feature places
    ):
        ...
        if mask_ids is None:
            mask_ids = torch.full_like(input_ids, -1)
        ...
        mask_ids = [
            cur_mask_ids[cur_attention_mask]
            for cur_mask_ids, cur_attention_mask in zip(mask_ids, attention_mask)
        ]
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if num_images == 0:
                ...
                new_mask_ids.append(mask_ids[batch_idx])
                cur_image_idx += 1
                continue
            ...
            cur_labels_noim = []
            cur_mask_ids = mask_ids[batch_idx]
            cur_mask_ids_noim = []
            ...
            for i in range(num_images + 1):
                ...
                cur_new_mask_ids.append(cur_mask_ids_noim[i])
                ...
                if i < num_images:
                    ...
                    cur_new_mask_ids.append(
                        torch.full(
                            (cur_image_features.shape[0],),
                            IMAGE_TOKEN_INDEX,
                            device=cur_mask_ids.device,
                            dtype=cur_mask_ids.dtype,
                        )
                    )
                    max_pos_id += cur_image_features.shape[0]

            cur_new_input_embeds = [
                x.to(device=cur_input_embeds.device) for x in cur_new_input_embeds
            ]
            cur_new_input_embeds = torch.cat(cur_new_input_embeds)
            cur_new_labels = torch.cat(cur_new_labels)
            cur_new_mask_ids = torch.cat(cur_new_mask_ids)

            new_input_embeds.append(cur_new_input_embeds)
            new_labels.append(cur_new_labels)
            new_mask_ids.append(cur_new_mask_ids)
        ...
        if tokenizer_model_max_length is not None:
            ...
            new_mask_ids = [x[:tokenizer_model_max_length] for x in new_mask_ids]
        ...
        new_mask_ids_padded = torch.full(
            (batch_size, max_len),
            -1,
            dtype=new_mask_ids[0].dtype,
            device=new_mask_ids[0].device,
        )
        ...
        for i, (cur_new_embed, cur_new_labels, cur_new_mask_ids) in enumerate(
            zip(new_input_embeds, new_labels, new_mask_ids)
        ):
            ...
            if getattr(self.config, "tokenizer_padding_side", "right") == "left":
                ...
                if cur_len > 0:
                    ...
                    new_mask_ids_padded[i, -cur_len:] = cur_new_mask_ids
        ...
        image_places = new_mask_ids_padded == IMAGE_TOKEN_INDEX
        new_mask_ids_padded[image_places] = -1

        if _mask_ids is None:
            new_mask_ids = None
        else:
            new_mask_ids = new_mask_ids_padded
