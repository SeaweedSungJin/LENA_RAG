...
class OpenCLIPVisionTower(nn.Module):
    def __init__(self, vision_tower, args, delay_load=False):
        ...
        self.vision_config = json.load(
            open(os.path.join(vision_tower, "open_clip_config.json"), "r")
        )
    ...
    def load_model(self):
        ...
        if "convnext" in self.vision_tower_name:
            if "large" in self.vision_tower_name and (
                "d-320" in self.vision_tower_name or "d_320" in self.vision_tower_name
            ):
        ...
        clip_model = CLIP(
            **get_model_config(self.model_type), drop_path=self.is_droppath
        )
