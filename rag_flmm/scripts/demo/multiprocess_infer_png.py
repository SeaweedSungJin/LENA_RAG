...
def do_kmeans(feature_map, gt_mask):
    ...
    masks = F.interpolate(
        torch.stack([mask1, mask2])[None], size=gt_mask.shape, mode="bilinear"
    )[0]
    ious = compute_mask_IoU(
        masks.view(2, -1), torch.from_numpy(gt_mask).float().view(1, -1)
    )[-1]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", default="/home/hrkim/f-lmm/output", type=str)
    ...
    dataset_params = dict(
        json_file="/home/hrkim/dataset/coco/annotations/png_coco_val2017.json",
        panoptic_json_file="/home/hrkim/dataset/coco/annotations/panoptic_val2017.json",
        panoptic_png_path="/home/hrkim/dataset/coco/annotations/panoptic_val2017",
        tokenizer=tokenizer,
        image_processor=image_processor,
        prompt_template=prompt_template,
        local_path="/home/hrkim/dataset/coco/val2017",
    ...
    with accelerator.split_between_processes(data_ids) as sub_ids:
        for idx in tqdm(sub_ids, disable=not accelerator.is_main_process):
            ...
            pred_masks = (
                F.interpolate(
                    output["pred_masks"][None].float().cpu(),
            ...
            sam_pred_masks = (
                F.interpolate(
                    output["sam_pred_masks"][None].float().cpu(),
                    size=gt_masks.shape[-2:],
                    mode="bilinear",
                )[0].numpy()
                > 0
            ...
            attn_masks = torch.stack(
                [
                    do_kmeans(mask_attention, gt_mask)
                    for mask_attention, gt_mask in zip(mask_attentions, gt_masks)
                ]
            )
            ...
            for color_id, (gt_mask, sam_mask, cnn_mask, attn_mask) in enumerate(
                ...
                sam_image[sam_mask] = (
                    ...
                    + np.array(colors[color_id]).reshape((1, 1, 3)) * 0.8
                )
                gt_image[gt_mask] = (
                    gt_image[gt_mask] * 0.2
                    + np.array(colors[color_id]).reshape((1, 1, 3)) * 0.8
                )
                conv_image[cnn_mask] = (
            ...
            all_in_one = np.concatenate(
                [image, attn_image, conv_image, sam_image, gt_image], axis=1
            )
            ...
            np.save(
                os.path.join(args.save_dir, f"attn_all/{file_name[:-4]}.npy"),
                mask_attentions.cpu().numpy(),
            )
