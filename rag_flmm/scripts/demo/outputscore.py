...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--image", default="/home/hrkim/f-lmm/27559056953552522.jpg", type=str
    )
    parser.add_argument(
        "--text", default="What is the model name of this car?", type=str
    )
    parser.add_argument(
        "--checkpoint",
    ...
    output_score = output.pop("output_scores")
    # print(output_score)
    output_ids = output.pop("output_ids").cpu()
    # softmax를 이용해 각 토큰에 대한 confidence score 계산
    confidences = []
    for logit, token_id in zip(output_score, output_ids):
        prob = F.softmax(logit, dim=-1)
        confidence = prob[0, token_id]  # 선택된 토큰의 확률
        confidences.append(confidence.item())

    # 전체 평균 confidence
    avg_conf = sum(confidences) / len(confidences)
    print(f"[Confidence] 평균 확신도: {avg_conf * 100:.2f}%")

    # 원하면 토큰별 confidence도 출력
    tokens = model.tokenizer.convert_ids_to_tokens(output_ids)
    for token, conf in zip(tokens, confidences):
        print(f"{token}\t{conf:.2%}")
