...
def mask2point(mask, image_h, image_w):
    ...
    ys, xs = np.where(mask)
    ys, xs = (
        image_h * (ys.astype(np.float32) + 0.5) / h,
...
if __name__ == "__main__":
    ...
    with accelerator.split_between_processes(data_ids) as sub_ids:
        for idx in tqdm(sub_ids, disable=not accelerator.is_main_process):
            ...
            pred_masks = F.interpolate(
                pred_mask_logits[None].float().sigmoid(),
                size=masks.shape[-2:],
                mode="bilinear",
            )[0].cpu()
            ...
            sub_mask_ious = [
                compute_mask_IoU(pred_masks.flatten(1, 2), gt_masks.flatten(1, 2))[-1]
            ]
            sub_isthing = [
                torch.tensor([mask_info["isthing"] for mask_info in mask_infos])
            ]
            sub_plural = [
                torch.tensor([mask_info["plural"] for mask_info in mask_infos])
            ]
    ...
    if accelerator.is_main_process:
        ...
        print(
            f"aIoU: {AA}, aIoU_singulars: {AA_singulars}, aIoU_plurals: {AA_plurals}, "
            f"aIoU_things: {AA_things}, aIoU_stuff: {AA_stuff}, aAcc@0.5: {accuracy}, "
            f"pixel_accs: {pixel_accs}",
            flush=True,
        )
