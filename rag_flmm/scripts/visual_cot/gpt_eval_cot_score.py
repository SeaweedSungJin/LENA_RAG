...
def get_score(question_text, gt_answer_text, pred_answer_text):
    ...
    time.sleep(
        1
    )  # sleep for 1 second after a successful request to avoid high frequency
    return res
...
if __name__ == "__main__":
    ...
    for result_file in result_files:
        ...
        for data_sample in tqdm(data):
            score = get_score(
                data_sample["question"], data_sample["gt"], data_sample["answer"]
            )
            scores.append(score)
            data_sample["score"] = score

        print(
            f"The avg score on {result_file} is: {sum(scores) / len(scores)}",
            flush=True,
        )
