...
sys.path.append("/home/hrkim/f-lmm")
...
def draw_mask(image, mask):
    ...
    image[mask] = (
        image[mask] * 0.5
        + np.array([255, 0, 0], dtype=np.float32).reshape(1, 1, 3) * 0.5
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--checkpoint",
        default="checkpoints/frozen_deepseek_vl_1_3b_unet_sam_l_iter_95080.pth",
        type=str,
    )
    parser.add_argument("--image_folder", default="/home/hrkim/dataset", type=str)
    ...
    args.save_folder = os.path.join(
        args.save_folder, f"{model_name}_visual_cot_{args.version}"
    )
    if args.debug:
    ...
    json_files = glob("/home/hrkim/f-lmm/scripts/visual_cot/benchmark/*.json")
    for json_file in json_files:
        ...
        # ious = []
        os.makedirs(
            os.path.join(args.save_folder, f"{os.path.basename(json_file)[:-4]}"),
            exist_ok=True,
        )
        with accelerator.split_between_processes(data_ids) as sub_ids:
            for idx in tqdm(sub_ids, disable=not accelerator.is_main_process):
                ...
                image = Image.open(
                    os.path.join(args.image_folder, data_sample["image"][0])
                )
                ...
                thought, box, answer, mask = getattr(
                    model, f"visual_cot_{args.version}"
                )(image, question, gt_bbox)
        ...
        if accelerator.is_main_process:
            ...
            with open(
                os.path.join(args.save_folder, os.path.basename(json_file)), "w"
            ) as f:
