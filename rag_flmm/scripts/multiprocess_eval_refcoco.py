...
if __name__ == "__main__":
    ...
    print(
        f"Finished moving model to device: {accelerator.device}, time used: {time() - tik}",
        flush=True,
    )
    model.eval()
    ...
    refcoco_root = "/home/hrkim/dataset/refer_seg/"

    refcoco_subsets = dict()
    for split in ["val", "testA", "testB"]:
        refcoco_subsets[f"refcoco_{split}"] = dict(
            ann_file=f"{refcoco_root}/refcoco/instances.json",
            split_file=f"{refcoco_root}/refcoco/refs(unc).p",
            split=split,
        )
    ...
    for split in ["val", "test"]:
        refcoco_subsets[f"refcocog_{split}"] = dict(
            ann_file=f"{refcoco_root}/refcocog/instances.json",
            split_file=f"{refcoco_root}/refcocog/refs(umd).p",
            split=split,
        )
