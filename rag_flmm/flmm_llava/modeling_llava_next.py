...
class CustomLlavaNextForConditionalGeneration(LlavaNextForConditionalGeneration):
    ...
    def _merge_input_ids_with_image_features(
        ...
        if mask_ids is not None:
            final_mask_ids[batch_indices, text_to_overwrite] = mask_ids[
                batch_indices, non_image_indices
            ]
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None and input_ids.shape[1] != 1:
                ...
                clip_image_features = torch.split(
                    selected_image_feature, split_sizes, dim=0
                )
                ...
                for image_idx, (image_feature, clip_image_feature) in enumerate(
                    ...
                    if image_feature.shape[0] > 1:
                        ...
                        clip_image_feature = clip_image_feature.permute(
                            4, 0, 2, 1, 3
                        ).contiguous()
                        clip_image_feature = clip_image_feature.flatten(1, 2).flatten(
                            2, 3
                        )
                        clip_image_feature = unpad_image(
                            clip_image_feature, image_sizes[image_idx]
                        )
