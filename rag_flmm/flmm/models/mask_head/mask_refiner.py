...
class SAMWrapper(nn.Module):
    def __init__(
        ...
        self.model = sam_model_registry[model_name](checkpoint=None)
        # Gracefully handle missing checkpoint during inference-only/routing use.
        try:
            if checkpoint is None:
                raise FileNotFoundError("SAM checkpoint is None")
            self.model.load_state_dict(torch.load(checkpoint, map_location="cpu"))
        except Exception as e:
            print(f"[SAMWrapper] Warning: failed to load checkpoint '{checkpoint}': {e}. "
                  f"Continuing without pre-trained SAM weights.")
        self.model.eval()
    ...
    def generate_prompt_masks(self, masks, input_size):
        ...
        masks = F.interpolate(
            masks[:, None].float(), size=input_size, mode="bilinear"
        ).to(masks)
        h, w = masks.shape[-2:]
        masks = F.pad(
            masks,
            (
                0,
                self.model.image_encoder.img_size - w,
                0,
                self.model.image_encoder.img_size - h,
            ),
            value=pad_value,
        )
        prompt_masks = F.interpolate(
            masks.float(), size=(256, 256), mode="bilinear"
        ).to(masks)
    ...
    def forward(self, image, pred_masks, text_embeds):
        ...
        pred_masks = F.interpolate(
            pred_masks.detach()[None].float().sigmoid(),
        ...
        sam_masks = []
        for prompt_mask, pred_mask, text_embed in zip(
            prompt_masks, pred_masks, text_embeds
        ):
            if self.use_box:
                if pred_mask.sum() > 0:
            ...
            if self.use_text:
                sparse_embeddings = torch.cat(
                    [
                        sparse_embeddings.to(dense_embeddings),
                        text_embed[None].to(dense_embeddings),
                    ],
                    dim=1,
                )
            else:
            ...
            if self.multimask_output:
                ...
                candidate_ious = compute_mask_IoU(
                    candidate_masks.view(3, -1), pred_mask.float().view(1, -1)
                )[-1]
