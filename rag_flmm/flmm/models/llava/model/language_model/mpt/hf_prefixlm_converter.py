...
def _expand_mask(
    ...
    """
    Expands attention mask from [bsz, src_len] to [bsz, 1, tgt_len, src_len] for attention broadcasting.
    """
    batch_size, src_len = mask.size()
    tgt_len = tgt_len if tgt_len is not None else src_len
    expanded_mask = mask[:, None, None, :].expand(batch_size, 1, tgt_len, src_len)
    return expanded_mask.to(dtype=dtype)
...
def _make_causal_mask(
    ...
    """
    Creates a lower-triangular causal attention mask.
    """
    ...
    if past_key_values_length > 0:
        mask = torch.cat(
            [torch.zeros(tgt_len, past_key_values_length, device=device), mask], dim=-1
        )
    return (
        mask[None, None, :, :]
