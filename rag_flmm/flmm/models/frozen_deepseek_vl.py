    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.image_token_idx = self.tokenizer.encode(
            "<image_placeholder>", add_special_tokens=False
        )[-1]
        print_log(f"Image token: {self.tokenizer.decode(self.image_token_idx)}")
        in_channels = (
            self.deepseek_vl.config.language_config.num_attention_heads
...
class FrozenDeepseekVLSAM(FrozenDeepseekVL):
    ...
    def _forward(self, data_sample):
        ...
        images_emb_mask = torch.ones(
            (1, 1, images_seq_mask.sum()),
            dtype=torch.bool,
        ...
        attentions = [
            attn.view(*attn.shape[:-1], self.clip_shape, self.clip_shape)
        ...
        with torch.no_grad():
            mask_attentions = F.interpolate(
                mask_attentions.float(), size=pred_masks.shape[-2:], mode="bilinear"
    ...
    def _prepare_for_generation(
        ...
        **kwargs,
        ...
        stop_words = self.prompt_template.get("STOP_WORDS", []) + [
            "."
        ]  # only need the first sentence
        self.stop_criteria = StoppingCriteriaList()
        self.stop_word_ids = [
            self.tokenizer.encode(word, add_special_tokens=False)[-1]
    ...
    def visual_cot_v1(self, image, question, *args, **kwargs):
        ...
        prompt = self.prompt_template["INSTRUCTION"].format(
            ...
            + question
            + "First think which object in this image is most relevant to the question."
        )
        ...
        images_emb_mask = torch.ones(
            (1, 1, 576), dtype=torch.bool, device=self.deepseek_vl.device
        )
        inputs_embeds = self.deepseek_vl.prepare_inputs_embeds(
            input_ids=input_ids,
            pixel_values=pixel_values,
        ...
        assert (
            len(output_ids) + inputs_embeds.shape[1] == past_key_values[0][0].shape[2]
        )
        ...
        attentions = [
            torch.cat(
                [attn[layer_id][0, ..., images_seq_indices] for attn in attentions],
                dim=-2,
            )
            for layer_id in range(
                self.deepseek_vl.language_model.config.num_hidden_layers
            )
        ]
        attentions = [
            attn.view(*attn.shape[:-1], self.clip_shape, self.clip_shape)
        ...
        mask_attentions = torch.cat(
            [self.apply_merge(attn, dim=1) for attn in attentions]
        ...
        pred_masks = F.interpolate(
            pred_masks[None].float(), size=(image.height, image.width), mode="bilinear"
        )[0].to(pred_masks)
        if self.use_sam:
        ...
        if self.with_memory:
            ...
            return (
                thought,
                bbox,
                self._conversation(conversation, [image, image_crop]),
                pred_mask,
            )
        else:
            return (
                thought,
                bbox,
                self.visual_cot_v3(image_crop, question)[-1],
                pred_mask,
            )
    ...
    def visual_cot_v2(self, image, question, *args, **kwargs):
        ...
        assert self._generation_ready
        prompt = self.prompt_template["INSTRUCTION"].format(
            input="<image_placeholder>" * 576 + question + "<image_placeholder>"
        )  # temporarily insert this special token to locate the question
        input_ids = self.tokenizer.encode(prompt, return_tensors="pt").to(
            self.deepseek_vl.device
        )
        ...
        mask_attentions = torch.cat(
            [
                self.apply_merge(attn[:, question_start_place:], dim=1)
                for attn in attentions
            ]
        )[None].to(self.mask_head.dtype)
    ...
    def visual_cot_v3(self, image, question, *args, **kwargs):
        ...
        return (
            ...
            self._conversation(conversation, [image]),
            None,
        )
    ...
    def answer(self, image, question, *args, **kwargs):
        ...
        attention_maps = torch.cat(
            [
                torch.cat(
                    [
                        attns[layer_id][0, ..., torch.where(images_seq_mask)[0]]
                        for attns in outputs.attentions[1:]
                    ],
                    dim=-2,
                )
            ...
            dim=0,
        ).view(num_layers * num_heads, -1, self.clip_shape, self.clip_shape)
        # collect embeddings
        text_layer_weights = self.get_text_layer_weights()
        hidden_states = torch.stack(
            [
                torch.cat(
                    [hs[layer_id + 1][0] for hs in outputs.hidden_states[1:]], dim=0
                )
                for layer_id in range(num_layers)
            ],
            dim=0,
    ...
    def ground(
        self, image, positive_ids, hidden_states, attention_maps, meta_data, **kwargs
    ):
        mask_attentions = []
        text_embeds = []
...
if __name__ == "__main__":
    ...
    _ = model.load_state_dict(
        guess_load_checkpoint(
            "checkpoints/frozen_deepseek_vl_1_3b_unet_sam_l_iter_95080.pth"
        ),
        strict=False,
    )
