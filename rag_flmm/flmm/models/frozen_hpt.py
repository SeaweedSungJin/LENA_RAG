...
class FrozenHPT(BaseModel):
    ...
    def _init_models(self, llm, visual_encoder, projector):
        ...
        if isinstance(visual_encoder, SiglipVisionModel):
            ...
            visual_encoder.vision_model.embeddings.num_patches = (
                self.image_size // patch_size
            ) ** 2
            visual_encoder.vision_model.embeddings.num_positions = num_positions
            visual_encoder.vision_model.embeddings.position_ids = torch.arange(
                num_positions
            ).expand((1, -1))
            visual_encoder.vision_model.embeddings.position_embedding.weight = (
                self.interpolate_pos_embed_siglip(visual_encoder, new_size)
            )
            ...
            visual_encoder.vision_model.embeddings.position_embedding.weight = (
                self.interpolate_pos_embed(visual_encoder, new_size)
            )
            visual_encoder.config.image_size = self.image_size
...
class FrozenHPTSAM(FrozenHPT):
    ...
    def _forward(self, data_sample):
        ...
        with torch.no_grad():
            pixel_values = data_sample["pixel_values"][None].to(
                device=self.visual_encoder.device, dtype=self.visual_encoder.dtype
            )
            visual_outputs = self.visual_encoder(
                pixel_values, output_hidden_states=True
            )
            pixel_values = self.projector(
                visual_outputs.hidden_states[self.visual_select_layer][
                    :, -self.num_patches :
                ].to(self.projector.dtype)
            ).to(self.llm.dtype)
    ...
    def _prepare_for_generation(
        self, image_processor, tokenizer, prompt_template, max_new_tokens=512, **kwargs
    ):
        raise NotImplementedError
