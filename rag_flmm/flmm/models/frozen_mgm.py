...
class FrozenMGM(BaseModel):
    def __init__(
        self, model, mask_head, merge="mean", loss_mask=None, loss_dice=None, **kwargs
    ):
        super().__init__()
        self._init_mgm_model(model)
        in_channels = (
            self.mgm.config.num_attention_heads * self.mgm.config.num_hidden_layers
        )
        if getattr(self.mgm.config, "image_grid", 1) > 1 and getattr(
            self.mgm.config, "image_global", False
        ):
            in_channels *= 2
    ...
    def clip_shape(self):
        return (
            self.image_processor.image_size_raw["height"] // self.patch_size,
    ...
    def _process_image(self, image):
        ...
        if hasattr(self.mgm.config, "image_size_aux"):
            ...
            image_tensor = torch.nn.functional.interpolate(
                image_tensor, size=raw_shape, mode="bilinear", align_corners=False
            )
        else:
            image_tensor_aux = []
    ...
    def _process_attention(self, attention2image):
        ...
        if image_grid == 1:
            ...
            if use_global_image:
                ...
                attention2global_image = attention2global_image.view(
                    num_heads, seq_len, *self.clip_shape
                )
            ...
            attention2hd_image = attention2hd_image.view(
                num_heads, seq_len, image_grid, image_grid, *self.clip_shape
            )
            attention2hd_image = attention2hd_image.permute(
                0, 1, 2, 4, 3, 5
            ).contiguous()
            attention2hd_image = attention2hd_image.view(
                num_heads,
                seq_len,
                image_grid * self.clip_shape[0],
                image_grid * self.clip_shape[1],
            )

            if attention2global_image is not None:
                attention2global_image = F.interpolate(
                    attention2global_image.float(),
                    scale_factor=image_grid,
                    mode="bilinear",
                ).to(attention2global_image)
                attention2hd_image = torch.cat(
                    [attention2global_image, attention2hd_image], dim=0
                )
...
class FrozenMGMSAM(FrozenMGM):
    def __init__(self, sam, *args, **kwargs):
        ...
        self.text_proj = nn.Linear(
            self.mgm.config.hidden_size, self.sam.model.prompt_encoder.embed_dim
        )
    ...
    def _forward(self, data_sample):
        ...
        image_tensor, image_tensor_aux = self._process_image(
        ...
        attentions = [
            attn[0, ..., outputs.image_places[0]] for attn in outputs.attentions
        ]
        ...
        for mask_id in range(len(masks)):
            ...
            mask_attentions.append(
                torch.cat(
                    [
                        self.apply_merge(
                            self._process_attention(attn[:, matched]), dim=1
                        )
    ...
    def _prepare_for_generation(
        self, tokenizer, prompt_template, max_new_tokens=512, **kwargs
    ):
        raise NotImplementedError
