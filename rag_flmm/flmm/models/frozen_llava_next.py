...
class FrozenLlavaNext(BaseModel):
    def __init__(
        ...
        **kwargs,
        ...
        in_channels = (
            self.llava.config.text_config.num_attention_heads
...
class FrozenLlavaNextSAM(FrozenLlavaNext):
    ...
    def _forward(self, data_sample):
        ...
        attention_mask = torch.ones(
            inputs["input_ids"].shape, device=self.llava.device, dtype=torch.bool
        )
        with torch.no_grad():
            outputs = self.llava(
                **inputs,
        ...
        coarse_image_feature_h, coarse_image_feature_w = (
            coarse_image_h // self.patch_size,
            coarse_image_w // self.patch_size,
        )
        ...
        for mask_id in range(len(masks)):
            ...
            mask_attentions_with_coarse = torch.cat(
                [
                    ...
                    for attn in attentions_with_coarse
                ]
            )
            mask_attentions_with_fine = torch.cat(
                [
                    self.apply_merge(attn[:, matched], dim=1)
                    for attn in attentions_with_fine
                ]
            )
        ...
        attention_maps = torch.cat(
            [
                F.interpolate(
                    attentions_with_coarse.float(),
                    size=(fine_image_feature_h, fine_image_feature_w),
                    mode="bilinear",
                ),
                F.interpolate(
                    attentions_with_fine.float(),
                    size=(fine_image_feature_h, fine_image_feature_w),
                    mode="bilinear",
                ),
            ],
    ...
    def compute_loss(self, data):
        ...
        for data_sample in data:
            ...
            gt_masks = F.interpolate(
                masks.to(pred_masks)[None].float(), size=pred_masks.shape[-2:]
            )[0].to(self.llava.dtype)
            assert pred_masks.shape == gt_masks.shape
            sam_gt_masks = F.interpolate(
                masks.to(sam_pred_masks)[None].float(), size=sam_pred_masks.shape[-2:]
            )[0].to(self.llava.dtype)
            mask_cnts += mask_cnt
