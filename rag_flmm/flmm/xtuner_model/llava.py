...
from flmm.utils import get_torch_device
from flmm.xtuner_model.modules import ProjectorConfig, ProjectorModel, dispatch_modules
from flmm.xtuner_model.modules.dispatch import SUPPORT_FLASH1, SUPPORT_FLASH2
from flmm.xtuner_model.utils import (
    LoadWoInit,
...
class LLaVAModel(BaseModel):
    def __init__(
        ...
        **kwargs,
        ...
        in_channels = (
            self.llm.config.num_attention_heads * self.llm.config.num_hidden_layers
        )
        mask_head.update(in_channels=in_channels)

        self.mask_head = BUILDER.build(mask_head)

        if pretrained_mask_head is not None:
            mask_ckpt = torch.load(pretrained_mask_head, map_location="cpu")
            print(type(mask_ckpt))

            if isinstance(mask_ckpt, dict):
                print(mask_ckpt.keys())

            # state_dict 키 여부 확인
            if "state_dict" in mask_ckpt:
                raw_state_dict = mask_ckpt["state_dict"]
            else:
                raw_state_dict = mask_ckpt
            # import ipdb; ipdb.set_trace()
            mask_state_dict = {
                k.replace("mask_head.", ""): v
                for k, v in raw_state_dict.items()
                if k.startswith("mask_head.")
            }
            self.mask_head.load_state_dict(mask_state_dict, strict=True)

        self.mask_head.requires_grad_(False)
    ...
    def forward(self, data_samples=None, mode="loss", **data):
        ...
        if "pixel_values" in data:
            # print("[DEBUG] pixel_values shape:", data["pixel_values"].shape)
            ...
            # allowed_keys = {"input_ids", "position_ids","past_key_values","labels", "pixel_values", "attention_mask"}  # 실제 필요한 키만
    ...
    def compute_loss(self, data, data_samples=None):
        if "labels" not in data or data["labels"] is None:
            print("[ERROR] labels are missing from data!")
        outputs = self.llm(**data)
        print("[DEBUG] outputs.loss =", outputs.loss)
        print("[DEBUG] type =", type(outputs.loss))
    ...
    def to_xtuner_llava(self, cfg, save_dir, fp32=False, save_pretrained_kwargs={}):
        ...
        if self.use_visual_encoder_lora:
            ...
            print_log(
                f"Saving visual_encoder image_processor to{visual_encoder_path}",
                "current",
            )
    ...
    def to_huggingface_llava(
        ...
        if added_token > 0:
            ...
            model.language_model.model.embed_tokens.weight.data[ori_vocab_size:] = (
                torch.stack(
                    tuple(
                        dist.sample()
                        for _ in range(
