...
class ExpertShard(nn.Module):
    def __init__(self, config, expert_in_one_shard=1):
        ...
        self.w1w3 = nn.Parameter(
            torch.empty(
                expert_in_one_shard, config.intermediate_size * 2, config.hidden_size
            )
        )
        self.w2 = nn.Parameter(
            torch.empty(
                expert_in_one_shard, config.hidden_size, config.intermediate_size
            )
        )
...
class MixtralSparseShardMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        for shard_index in range(self.shard_num):
            mask = expert_mask[
                shard_index * self.expert_in_one_shard : (shard_index + 1)
...
def _load_pretrained_model(
    ...
    if (
        ...
        or (resolved_archive_file is None)
        or (low_cpu_mem_usage)
        or (device_map is not None)
        or (offload_folder is not None)
        or (not (offload_state_dict is None or offload_state_dict is False))
        or (hf_quantizer is not None)
        or (keep_in_fp32_modules is not None and len(keep_in_fp32_modules) > 0)
