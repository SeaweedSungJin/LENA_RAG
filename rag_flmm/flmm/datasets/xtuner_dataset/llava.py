...
class LLaVADataset(Dataset):
    def __init__(
        ...
        # RAG routing token
        self.no_rag_token = "[NO_RAG]"
        self.tokenizer.add_special_tokens(
            {"additional_special_tokens": [self.no_rag_token]}
        )
        self.no_rag_token_id = self.tokenizer.convert_tokens_to_ids(self.no_rag_token)
    ...
    def __getitem__(self, index):
        ...
        # [NO_RAG] 토큰 추가 (input_ids 기준)
        if "input_ids" in data_dict and self.tokenizer is not None:
            data_dict["input_ids"] = [self.no_rag_token_id] + data_dict["input_ids"]

            # labels도 있으면 -100 추가해서 loss 계산 제외
            if "labels" in data_dict:
                data_dict["labels"] = [-100] + data_dict["labels"]

            # ✅ 라우터 학습을 위한 rag_label 추가
            data_dict["rag_label"] = 0  # LLaVA는 항상 NO_RAG
