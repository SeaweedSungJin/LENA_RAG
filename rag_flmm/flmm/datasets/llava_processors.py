...
from transformers.models.clip.image_processing_clip import CLIPImageProcessor, logger
...
class CustomLlavaImageProcessor(CLIPImageProcessor):
    ...
    def pad(self, image):
        pad_value = np.array(
            tuple(int(x * 255) for x in self.image_mean), dtype=image.dtype
        )
        ...
        new_image[
            before_height : size - after_height, before_width : size - after_width
        ] = image
