...
#         "rag_label": torch.tensor([x["rag_label"] for x in batch], dtype=torch.long),
#     }
#     # 추가: <image> 토큰 개수 확인
#     image_token_id = (
#         batch[0]["pad_token_id"] + 1
#     )  # 정확한 값은 tokenizer로 다시 변환 필요
#     count = (result["input_ids"] == image_token_id).sum().item()
#     print(f"[DEBUG] Batch image token count: {count}")
#     return result
#     return {
#         "input_ids": torch.stack(input_ids),
#         "labels": torch.stack(labels),
...
def custom_collate_fn(batch):
    ...
    for s in batch:
        ...
        pad = max_len - T
        input_ids.append(torch.cat([s["input_ids"],
                                    torch.full((pad,), pad_token_id, dtype=torch.long)]))
        labels.append(torch.cat([s["labels"],
                                 torch.full((pad,), IGNORE_INDEX, dtype=torch.long)]))

    input_ids = torch.stack(input_ids, dim=0)     # [B,T]
    labels    = torch.stack(labels,    dim=0)     # [B,T]

    # 2) attention_mask (패딩 0)
    attention_mask = input_ids.ne(pad_token_id)   # [B,T], bool

    # 3) 이미지 스택
    pixel_values = torch.stack([s["pixel_values"] for s in batch], dim=0)  # [B,C,H,W]

    # 4) rag_label: 텐서 [B]로
    rag_label = torch.tensor([int(s["rag_label"]) for s in batch], dtype=torch.long)
    ...
    # (선택) 메타 유지
    for k in ("question", "answer", "data_id", "image_path", "kb_text"):
        if k in batch[0]:
            out[k] = [s.get(k, None) for s in batch]

    # 디버그: 이미지 토큰 카운트 (정확한 image_token_id가 있을 때만)
    if "image_token_id" in batch[0]:
        img_id = batch[0]["image_token_id"]
        cnt = (input_ids == img_id).sum().item()
        print(f"[DEBUG] image_token occurrences in batch: {cnt}")
...
class PNGDataset(Dataset):
    def __init__(
        ...
        self.prompt = self.tokenizer.encode(
            prompt_template["INSTRUCTION"].format(input=prompt), add_special_tokens=True
        )
    ...
    def __getitem__(self, index):
        ...
        for segment in data_sample["segments"]:
            segment_input_ids = self.tokenizer.encode(
                segment["utterance"], add_special_tokens=False
            )
            ...
            if len(segment["segment_ids"]) == 0:
                ...
                if not segment["plural"]:
                    ...
                    isthing = self.coco.cats[annotations[segment_id]["category_id"]][
                        "isthing"
                    ]

                else:
                    isthing = 1
                mask_infos.append(dict(plural=segment["plural"], isthing=isthing > 0))
                # todo: load masks
                mask_cnt += 1
        ...
        padded_masks[
            :,
            padding["before_height"] : p_h - padding["after_height"],
            padding["before_width"] : p_w - padding["after_width"],
        ] = masks
...
if __name__ == "__main__":
    ...
    dataset = PNGDataset(
        json_file="data/png_coco_val2017.json",
