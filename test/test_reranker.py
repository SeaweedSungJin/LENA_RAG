...
def run_test(
    ...
    **kwargs,
    ...
    if kwargs["perform_vqa"]:
        ...
        llm_device_map = kwargs.get("llm_device_map", "single")
        llm_device_map = None if llm_device_map == "single" else "auto"
        llm_max_memory = kwargs.get("llm_max_memory", None)
        llm_dtype = kwargs.get("llm_dtype", "float16")
        llm_8bit = kwargs.get("llm_load_in_8bit", False)
        llm_4bit = kwargs.get("llm_load_in_4bit", False)
        if ag in ["llama3", "llama", "llama3.1", "llama-3.1", "llama-3.1-8b"]:
            if not llm_ckpt:
                raise ValueError("--llm_checkpoint is required for LLaMA3 answer generator")
    ...
    if kwargs["perform_qformer_reranker"]:
        ...
        blip_model, vis_processors, txt_processors = load_model_and_preprocess(
            name="blip2_reranker", model_type="pretrain", is_eval=True, device="cpu"
        )
        ...
        blip_model = blip_model.half().to(qformer_device)
    ...
    print(
        f"[Slice] dataset_start={start_idx}, dataset_end={end_idx}, dataset_limit={limit_raw if limit_raw is not None else 'None'} -> total {len(selected_indices)}"
    )
    progress = tqdm(
    ...
    retrieval_result = {}
    for processed_idx, dataset_idx in progress:
        ...
        print("wiki_url: ", example["wikipedia_url"]) ; print("question: ", example["question"]) 
        router_prob = None
        router_backend = None
        use_rag = True
        if router is not None:
            try:
                ...
                print(f"[Router] prob={router_prob:.3f} (threshold={decision.threshold:.3f}) backend={router_backend} -> use_rag={use_rag}")
            except Exception as exc:
                print(f"[Router] failed to obtain decision ({exc}); defaulting to use_rag=True")
                use_rag = True
        if not use_rag:
    ...
    if router is not None:
        total_routed = router_true_count + router_false_count
        print("=== Router Summary ===")
        if total_routed > 0:
            rate = (router_true_count / total_routed) * 100.0
            print(
                f"Use RAG: {router_true_count}/{total_routed} ({rate:.2f}%) | "
                f"Skip RAG: {router_false_count}/{total_routed} ({100.0-rate:.2f}%)"
            )
        else:
            print("No router decisions were made.")
    else:
        print("=== Router Summary ===")
    ...
    if kwargs.get("perform_vqa", False) and vqa_total_count > 0:
        ...
        print("Total examples:", vqa_total_count)
        print("Correct (BEM>=0.5):", vqa_correct_count)
        print(f"Accuracy: {acc*100:.2f}%")
        print(f"Average BEM: {avg_bem:.4f}")
        # Terminate run_test here to avoid executing any legacy code paths
        # that might duplicate evaluation or mutate counters.
        return
        if metric == "answer matching":
            entry_articles = [reconstruct_wiki_article(entry) for entry in entries]
            found = False
        ...
        if kwargs["perform_qformer_reranker"]:
            ...
            with torch.cuda.amp.autocast():
                ...
                # smaller text batch to reduce VRAM; configurable via --qformer_batch
                rerank_step = int(kwargs.get("qformer_batch", 32))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--nli_section_limit", type=int, default=10)
    parser.add_argument("--nli_context_sentences", type=int, default=3)
